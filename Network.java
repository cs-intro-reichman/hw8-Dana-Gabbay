...
public class Network {
    ...
    *  Otherwise, creates a new user with the given name, adds the user to this network, and returns true. */
    public boolean addUser(String name) {
        if (this.userCount==this.users.length+1){
            return false;
        }
        for (int i=0; i<this.userCount;i++)
        {
        ...
        this.users[this.userCount+1]= new User(name);
    ...
    public String mostPopularUser() {
        User popular = this.users[0];
        int max = this.followeeCount(this.users[0].getName());
        for (int i=1; i<this.userCount; i++){
            int temp = this.followeeCount(this.users[i].getName());
            if (max<temp){
                max = temp;
                popular = this.users[i];
            }
        }
